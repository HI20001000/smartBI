...
class LLMChatSession:
    ...
    def normalize_sql_user_input_with_llm(self, user_input: str) -> dict:
        ...
        def _apply_common_typos(text: str) -> str:
            normalized = text
            typo_pairs = [
                ("生名", "姓名"),
                ("姓明", "姓名"),
        ...
        prompt = [
            SystemMessage(
                content=(
                    "你是 SmartBI 查詢語句修正器。"
                    "請將使用者查詢中的明顯錯字、同音錯字、全半形、大小寫、空白與常見中英混用詞彙修正成更標準的查詢文字。"
                    "不要改變原始意圖，不要新增未提及的條件。"
                    "若是欄位詞錯字，優先修正為銀行查詢常見詞，例如：姓名、電子郵件、電話、客戶編號、帳號。"
                    "請特別注意：生名=>姓名、鄱件=>郵件。"
                    "只輸出 JSON，格式固定為："
                    '{"normalized_input":"..."}'
                )
            ...
            HumanMessage(
                content=(
                    f"user_input={normalized_input}\n"
                    "請先做拼字修正，再輸出 JSON。\n"
                    "示例1：查詢客戶10001 生名與鄱件 -> 查詢客戶10001 姓名與郵件\n"
                    "示例2：check client 10001 name and mail -> 查詢客戶10001 姓名與電子郵件"
                )
            ),
        ]
        ...
        normalized_input = _apply_common_typos(normalized_input)
