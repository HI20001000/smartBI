        ...
        prompt = [
            SystemMessage(
                content=(
                    ...
                    "3) time_start/time_end 格式為 yyyy-mm-dd；若無法判定則輸出空字串 \"\"。\n"
                    ...
                    "\n"
                    "時間解析規則：\n"
                    "- 若出現明確日期範圍：例如 \"2024-01-01 到 2024-01-31\"，填入 time_start/time_end。\n"
                    "- 若出現月份/季度/年份：\n"
                    "  * \"2024年1月\" => time_start=2024-01-01, time_end=2024-01-31（按自然月最後一天）\n"
                    "  * \"2024年Q1\" => time_start=2024-01-01, time_end=2024-03-31\n"
                    "- 若出現相對時間：今天/昨天/近7天/最近一個月/本月/上月/今年/去年：\n"
                    "  * 你【不知道當前日期】時，不要換算成具體日期，time_start/time_end 留空，並把相對時間詞放到 tokens 或 filters（例如：\"近7天\"）。\n"
                    "  * 若輸入本身包含明確“以某日為基準”的日期，才可換算。\n"
                    "\n"
                    "分類優先級（避免亂放）：\n"
                    "1) 能明確聚合/指標 => metrics\n"
